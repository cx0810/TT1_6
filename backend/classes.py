...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:H3ll02208-sp@localhost:3306/tt16'
...
class User(db.Model):
    ...
    user_id = db.relationship(
        'Itinerary', backref='User', lazy=True, passive_deletes=True)
...
class Itinerary(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    Itinerary_id = db.relationship(
        'ItineraryDestination', backref='Itinerary', lazy=True, passive_deletes=True)
...
class Country(db.Model):
    ...
    country_id = db.relationship(
        'Itinerary', backref='country', lazy=True, passive_deletes=True)
    destination_id = db.relationship(
        'Destination', backref='country', lazy=True, passive_deletes=True)
...
class Destination(db.Model):
    ...
    country_id = db.Column(db.Integer, db.ForeignKey(
        'country.id'), nullable=False)
    destination_id = db.relationship(
        'ItineraryDestination', backref='destination', lazy=True, passive_deletes=True)
